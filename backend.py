

import os
import numpy as np
import cv2
import face_recognition

class Utility:
    
     #check_for_user_prev_enc
        #if prev = None:
        #   get_user_encoding
        #   get_all_encodings
        #   update_encodings
        #   savedata
        #else:
        #   get_user_encoding
        #   get_prev_encoding
        #   update_user_encoding
        #   update_encodings
        #   savedata
        # 
    def __init__(self, personName, imageFiles):
        self.personName = personName
        self.imageFiles = imageFiles
        self.data_file_exist = False
    @property
    def person_name(self):
        return self.personName 
    @property
    def images(self):
        return self.imageFiles
    
    
    @person_name.setter
    def person_name(self, name):
        self.personName = name

    @images.setter
    def images(self,images):
        self.imageFiles = images

    def encode(self, imagePath: str):
        #image contents as numpy array
        #print(imagePath)
        image = face_recognition.load_image_file(imagePath)

        #print(image)

        #A list of 128-dimensional face encodings (one for each face in the image)
        #np.array
        try:
            image_encoding = face_recognition.face_encodings(image)[0]
        #print(image_encoding)
        except IndexError as e:
            print(e)
            return "no_face"
        return image_encoding
    
    def get_user_encoding(self,personName:str):
        data = np.load('data.npz',allow_pickle=True)
        #breakpoint()
        

        arr_enc = data[personName]
        return arr_enc

    def userHasEnc(self,personName: str):
        data = np.load('data.npz')
        if personName in list(data.keys()):
            return True

        else:
            return False
    
    def get_all_enc(self):
        """"""       
        return np.load('data.npz',allow_pickle = True)
    

    def savedata(self, args:dict):

        np.savez_compressed('data.npz',**args)

    def load_user_data(self):
        return np.load('data.npz')
    
    def checkIf_dataExist(self):
        self.data_file_exist =  os.path.isfile('data.npz')
    
   
# a = Utility("Jession",r"C:\Users\JDiwanga\OneDrive - Analog Devices, Inc\Documents\Training and Learning\Python Scripts\ml\mvc_practice\Splash_Screen_Python_PySide2\Menu.png")
# print(a)

class videoHandler:
    def __init__(self,source):
        self.video_source = source
    
    def setupWebcam(self):
        video_capture = cv2.VideoCapture(self.video_source)
        # Initialize some variables



        known_face_encodings = []
        known_face_names = []

        total_enc = []
        all_enc = np.load('data.npz',allow_pickle = True) #ndarray
        all_users = all_enc.files
        #print(all_users)
        #breakpoint()
        for user in all_users:
            
            temp_ndarr = all_enc[user]
            for temp_arr in temp_ndarr:
                known_face_names.append(user)
                known_face_encodings.append(temp_arr)
        

                
            

        face_locations = []
        face_encodings = []
        face_names = []
        process_this_frame = True

        while True:
            # Grab a single frame of video
            ret, frame = video_capture.read()

            # Resize frame of video to 1/4 size for faster face recognition processing
            small_frame = cv2.resize(frame, (0, 0), fx=0.25, fy=0.25)

            # Convert the image from BGR color (which OpenCV uses) to RGB color (which face_recognition uses)
            rgb_small_frame = small_frame[:, :, ::-1]

            # Only process every other frame of video to save time
            if process_this_frame:
                # Find all the faces and face encodings in the current frame of video
                face_locations = face_recognition.face_locations(rgb_small_frame)
                face_encodings = face_recognition.face_encodings(rgb_small_frame, face_locations)

                face_names = []
                for face_encoding in face_encodings:
                    # See if the face is a match for the known face(s)
                    matches = face_recognition.compare_faces(known_face_encodings, face_encoding)
                    name = "Unknown"

                    # # If a match was found in known_face_encodings, just use the first one.
                    # if True in matches:
                    #     first_match_index = matches.index(True)
                    #     name = known_face_names[first_match_index]

                    # Or instead, use the known face with the smallest distance to the new face
                    face_distances = face_recognition.face_distance(known_face_encodings, face_encoding)
                    best_match_index = np.argmin(face_distances)
                    if matches[best_match_index]:
                        name = known_face_names[best_match_index]

                    face_names.append(name)

            process_this_frame = not process_this_frame


            # Display the results
            for (top, right, bottom, left), name in zip(face_locations, face_names):
                # Scale back up face locations since the frame we detected in was scaled to 1/4 size
                top *= 4
                right *= 4
                bottom *= 4
                left *= 4

                # Draw a box around the face
                cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 2)

                # Draw a label with a name below the face
                cv2.rectangle(frame, (left, bottom - 35), (right, bottom), (0, 0, 255), cv2.FILLED)
                font = cv2.FONT_HERSHEY_DUPLEX
                cv2.putText(frame, name, (left + 6, bottom - 6), font, 1.0, (255, 255, 255), 1)

            # Display the resulting image
            cv2.imshow('Video', frame)

            # Hit 'q' on the keyboard to quit!
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break

        # Release handle to the webcam
        video_capture.release()
        cv2.destroyAllWindows()
            

        

